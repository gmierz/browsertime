...
class Firefox {
  ...
  async beforeEachURL(runner) {
    await runner.runPrivilegedScript(`
      Services.fog.testFlushAllChildren(); // force any data that wasn't recorded yet to be immediately put in the buffers
  ...
  async afterPageCompleteCheck(runner, index, url, alias) {
    ...
    let powerusage = await runner.runPrivilegedScript(`
      Services.fog.testFlushAllChildren(); // force data from child processes to be sent to the parent.
      return Glean.power.totalCpuTimeMs.testGetValue()
    `)
    log.info("sparky")
    log.info("sparky " + powerusage)
