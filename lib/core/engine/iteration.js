...
class Iteration {
  ...
  async run(navigationScript, index) {
    ...
    try {
      ...
      log.info(options.chrome.enableChromeDriverLog)
      log.info(options.chrome.enableVerboseChromeDriverLog)

      log.info(options.verbose || options.chrome.enableChromeDriverLog)
