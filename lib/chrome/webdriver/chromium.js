...
class Chromium {
  ...
  async afterPageCompleteCheck(runner, index, url, alias) {
    ...
    log.info("sparky")
    log.info(this.options.verbose)
    log.info(this.chrome.enableChromeDriverLog)
    log.info(this.chrome.enableVerboseChromeDriverLog)

    log.info(this.options.verbose >= 2 || this.chrome.enableChromeDriverLog)

    if (this.options.verbose >= 2 || this.chrome.enableChromeDriverLog) {
      log.info("doing something to chromedrivers")
